...
- `PYDANTIC_AGENT_MODEL_RETRY_COUNT`
  - Pydantic AI 模型调用、工具调用和结构化输出校验的最大重试次数，默认 `3` 次。
...
- 结构化输出模式
  ...
  - 模型调用、工具调用和结构化输出校验受 `model_retry_count` 限制，避免限流时反复重试。
...
$env:PYDANTIC_AGENT_MODEL_RETRY_COUNT = "3"
```
